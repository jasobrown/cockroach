...
LIBPMEMROACH := $(LIBROACH_DIR)/libpmemroach.a
...
$(LIBPMEMROACH_DIR)/Makefile: $(C_DEPS_DIR)/libpmemroach-rebuild | bin/.submodules-initialized
	...
	@# NOTE: If you change the CMake flags below, bump the version in
	@# $(C_DEPS_DIR)/libpmemroach-rebuild. See above for rationale.
	cd $(LIBPMEMROACH_DIR) && cmake $(xcmake-flags) $(LIBPMEMROACH_SRC_DIR) \
		-DCMAKE_BUILD_TYPE=$(if $(ENABLE_LIBPMEMROACH_ASSERTIONS),Debug,Release) \
		-DPROTOBUF_LIB=$(LIBPROTOBUF)
...
#TODO(JEB) not sure if i'll needd an entry here
...
c-deps-fmt:
	...
	find $(LIBPMEMROACH_SRC_DIR) -name '*.cc' -o -name '*.h' | xargs grep -L 'DO NOT EDIT' | xargs clang-format -i
